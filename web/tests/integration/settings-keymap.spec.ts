...
test("Settings applies Dvorak to an already-running Terminal", async ({ page }) => {
  ...
  // Launch Settings (launcher row four) and locate its focused accent frame.
  await clickFramebuffer(page, 40, TASKBAR_Y);
  ...
  const workersWithSettings = await workerCount(page);
  ...
  const settingsClose = titlebarControlPoint(settingsOrigin, "close");
  await clickFramebuffer(page, settingsClose.x, settingsClose.y);
  await expect
    .poll(() => workerCount(page), { timeout: 5_000 })
    .toBe(workersWithSettings - 1);
  const formerSettingsTask = taskbarEntryPoint(1, 2);
  await expect
    .poll(() => pixel(page, formerSettingsTask.x, formerSettingsTask.y), {
      timeout: 5_000,
    })
  ...
  const terminalAfterSettings = taskbarEntryPoint(0, 1);
  const terminalPalette = await pixel(
    page,
    terminalAfterSettings.x,
    terminalAfterSettings.y,
  );
  if (
  ...
  await expect
    .poll(() =>
      pixel(page, terminalAfterSettings.x, terminalAfterSettings.y), {
      timeout: 5_000,
    })
    .toEqual([...TASKBAR_LIGHT_FOCUSED]);
  await waitForActiveWindowBounds(page, {
    minimumWidth: 700,
    message: "Terminal did not present its focused frame after Settings closed",
  });
  for (const physicalKey of ["KeyD", "KeyB", "KeyG", "KeyK"]) {
    await pressAndWaitForFrame(page, physicalKey);
  }
  await page.keyboard.press("Enter");
  // `exit` removes both the Terminal client Worker and its separately isolated
  // persistent /bin/sh child, in addition to the Settings Worker closed above.
  await expect
    .poll(() => workerCount(page), { timeout: 5_000 })
    .toBe(workersWithSettings - 3);
  await expect
    .poll(() => {
      const point = taskbarEntryPoint(0, 1);
