...
- [ ] T231 M1.2 `SabBackend` implementing `KernelBackend` in `web/src/sab-backend.ts`. Writes requests into a `SharedArrayBuffer` ring + parks on `Atomics.wait`; in vitest (which runs under node) both sides busy-spin against a plain `ArrayBuffer`. +5 vitest tests in `web/tests/unit/sab-backend.test.ts` asserting byte-for-byte equivalence with `KernelWasmHostBackend` across FD_WRITE, FD_READ with pre-seeded input, PATH_OPEN ENOENT, PROC_EXIT, and heap-overflow rejection. Still no Workers; proves the backend swap is wire-equivalent.
