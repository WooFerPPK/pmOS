...
- [ ] T232 M1.3 `user-worker-entry.ts` + main-thread spawn choreography. New files: `web/src/user-worker-entry.ts` (Worker-scope entry waiting for `boot {pid, sab, wasmBytes}`; runs `UserWasmRuntime` against `SabBackend`; posts `{kind:"exited", pid, code}` on exit), `web/tests/unit/user-worker-entry.test.ts`, `web/tests/unit/bootstrap-spawn.test.ts`. Extends `web/src/shared/worker-proto.ts` with `proc:sab`, `proc:exited`, `proc:spawn` message kinds. Extends `crates/xtask/src/assemble_dist.rs` to bundle `user-worker-entry.ts` into `dist/assets/user-worker.js` (manifest entry count +1). +4-6 vitest tests using FakeWorker mocks to drive the message plumbing end-to-end without a real Worker.
