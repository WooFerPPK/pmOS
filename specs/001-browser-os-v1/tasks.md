...
- [ ] T230 M1.1 Kernel-side `kernel_service_sab` export. Add `#[no_mangle] extern "C" fn kernel_service_sab(pid, sab_ptr, sab_len, scratch_ptr, scratch_len) -> i32` in `crates/kernel/src/wasm_entry.rs` wrapping `Dispatcher::service_one` against a `ring::Sab::from_raw` view. Add `KernelWasmHost.serviceSab` in `web/src/kernel-wasm-host.ts`. +3 vitest tests at `web/tests/unit/kernel-wasm-host.test.ts`: FD_WRITE through the SAB ring fires `onConsoleWrite`; empty ring returns `1`; PROC_EXIT through the SAB retires the pid. No Workers yet. Budget impact: none (adds a memcpy-each-way wrapper; hot-path overhead measured in T220).
