...
- [ ] T082 Input driver in `web/src/drivers/input.ts` (listens to `keydown`/`keyup`/`mousemove`/`mousedown`/`mouseup`/`wheel` on the canvas, normalises to internal keycodes, pushes into two SAB rings for `/dev/input/kbd` and `/dev/input/mouse`) — **partial**: `web/src/drivers/input.ts` captures pointer events and routes them through the kernel Worker via `shared/input-proto.ts` (commit `d1fb1e3`). Keyboard capture, keycode normalisation, and the dual-SAB-ring push path (one per dev node) are deferred with T079.
- [ ] T083 [P] Input driver tests with synthetic DOM events in `web/tests/unit/input.test.ts` — **partial**: `web/tests/unit/input-driver.test.ts` + `input-proto.test.ts` cover the preview-slice pointer path; no keyboard coverage yet. No `input.test.ts` file.
