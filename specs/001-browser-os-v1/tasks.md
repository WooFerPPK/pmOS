...
- [ ] T091 Kernel Worker entrypoint `web/src/kernel-worker.ts`: instantiates the kernel WASM, wires the driver ports, starts the kernel's main loop — **partial**: `web/src/kernel-worker.ts` + `kernel-worker-entry.ts` exist (with `kernel-worker.test.ts` + `kernel-worker-entry.test.ts`) and run a mock-kernel preview loop (`web/src/mock-kernel.ts`) that accepts the same messages the real kernel will. Real WASM instantiation is blocked on the `wasm32-wasi` → `wasm32-wasip1` rustup rename documented in T029 — once the kernel can be built for the browser, this task swaps `mock-kernel.ts` for the real `kernel.wasm`.
