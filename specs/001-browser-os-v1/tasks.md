...
- [ ] T091 Kernel Worker entrypoint `web/src/kernel-worker.ts`: instantiates the kernel WASM, wires the driver ports, starts the kernel's main loop — **partial** (progressed by the extern-C seam slice on top of T073): `web/src/kernel-worker.ts` + `kernel-worker-entry.ts` exist (with `kernel-worker.test.ts` + `kernel-worker-entry.test.ts`) and run a mock-kernel preview loop (`web/src/mock-kernel.ts`) that accepts the same messages the real kernel will. The extern-C export seam the kernel Worker needs now exists at `crates/kernel/src/wasm_entry.rs`: nine exports (`kernel_init`, `kernel_register_process`, `kernel_install_console_fd`, `kernel_mark_running`, `kernel_dispatch`, plus the `kernel_{req,resp,heap}_ptr` + `kernel_heap_len` pointer getters) plus `memory` itself. The four host imports the `WasmPlatform` pulls in (`pmos_host_halt` / `_driver_call` / `_random_bytes` / `_panic`) are the entire import surface the host side has to satisfy. `kernel.wasm` compiles at 112 KiB now (was 86 bytes before the exports pinned the dispatcher alive through DCE) and is driven end-to-end by 17 vitest tests at `web/tests/unit/kernel-wasm-entry.test.ts` that load the cdylib from `target/wasm32-unknown-unknown/release/kernel.wasm`, stub the host imports, and exercise the 9 T073-vertical opcodes. The residual T091 work — the actual swap of `mock-kernel.ts` for a TS wrapper that calls into `kernel.wasm`, plus driver-port fan-in, plus the round-robin service loop over multiple process SABs — remains. The seam work in this slice made that a scoped TS-side refactor instead of a blocked-on-Rust-changes project.
