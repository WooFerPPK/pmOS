...
- [ ] T233 M1.4 Kernel Worker dispatch loop. `KernelWasmHost.startDispatchLoop(pidSource, halted): Promise<void>` in `web/src/kernel-wasm-host.ts` runs the round-robin service + wake-slot `Atomics.wait` (50 ms timeout). `kernel-worker-entry.ts`'s `runBootBinary` collapses to `PROC_SPAWN init + startDispatchLoop`. Adds `proc:sab` + `proc:exited` handlers on the kernel-worker messaging channel. Default `onSpawnProcess` stops queuing in-process spawns; it posts `proc:spawn` to main and returns the new pid. +6-8 vitest tests in `web/tests/unit/kernel-worker-entry.test.ts` (spawn choreography end-to-end) and `web/tests/unit/dispatch-loop.test.ts` (round-robin + 8-request budget + termination on all-pids-exited). Rust test count unchanged.
