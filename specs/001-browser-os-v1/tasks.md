...
- [ ] T075 `proc_wait` / `proc_kill` / signal delivery (signal inbox IPC channel, default dispositions, `SIGKILL`/`SIGTERM` semantics) in `crates/kernel/src/proc/signal.rs` — **partial**: `crates/kernel/src/proc/signal.rs` (170 lines) defines `Signal` + `SignalInbox` + coalescing, and `Kernel::proc_wait`/`proc_kill`/`drain_signals`/`pending_signals` exist on `Kernel` in `sys.rs` with coverage in `tests/sys.rs`. The userland-visible `FdObject::SignalChannel` fd (drain-by-`fd_read` on fd 3) is documented in `signal.rs` but not yet implemented; tests use `Kernel::drain_signals` as a direct back door.
...
- [ ] T110 Display server main loop: listens on `/run/display`, accepts clients, dispatches protocol messages, composites and presents at the framebuffer's refresh rate, in `crates/display-server/src/main.rs` — **not started**: `crates/display-server/src/main.rs` is a 29-line stub that constructs `Server::new()` and prints a diagnostic. The real `/run/display` accept loop is blocked on kernel IPC (T072 `ipc_*`) + `display_connect` (T072) + a working Worker-side ring (T091). Today the display server is driven entirely in-process by the unit tests and by the preview-slice loopback harness.
