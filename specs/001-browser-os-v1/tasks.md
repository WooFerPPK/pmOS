...
- [ ] T235 M1.6 Reconcile + delete the preview drain. `KernelWasmHost.drainPendingSpawns` + the `pendingSpawns` queue deleted. Existing vitest composition tests in `user-wasm-runtime.test.ts` / `kernel-wasm-host.test.ts` migrated onto `startDispatchLoop` with fake pid sources OR onto direct `KernelWasmHostBackend.dispatch` calls for tests that don't need the loop semantics. Update T091 + T074 deviation notes in `tasks.md` ("Done, landed in T230–T235"). Update parent `plan.md` Known Deviations block: acknowledge the new `user-worker.js` shipped asset; remove the preview in-process drain from the deviation register. Update SESSION-NOTES.md runway: demote the Multi-process item. Net test count delta: 0 (tests rewritten in place).
