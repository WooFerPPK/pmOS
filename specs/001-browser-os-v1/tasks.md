...
- [ ] T234 M1.5 Playwright browser integration. `bootstrap.ts`'s real-kernel mode swaps from the fake `onSpawnProcess`→drain combo to the real main-thread Worker spawner (allocate SAB, `new Worker("/assets/user-worker.js", {type:"module"})`, post `boot`, post `proc:sab` to the kernel Worker). Extends `web/tests/integration/real-kernel.spec.ts` assertions: `init` and `hello-std` run in different Workers (test-only hook exposes the live-pid Map size); the existing four-line ordered output still reaches `#pmos-real-console`. First slice with real user Workers running in the browser. TS-unit count delta: 0. Playwright: 0 new spec files, strengthened assertions.
