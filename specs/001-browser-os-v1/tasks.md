...
- [x] T029 ~~Verify `just build` produces a complete `dist/` tree~~ — **demo-path variant accepted**: an esbuild-only build of `web/src/bootstrap.ts` produces `dist/assets/bootstrap.js`; `dist/index.html`, `dist/sw.js`, `dist/_headers`, and `dist/manifest.json` are all committed; a zero-dependency Python static server at `serve-demo.py` serves `dist/` locally on http://localhost:8080 with the full COOP/COEP/CORP header set. End-to-end verified: `curl` against the server returns 200 for every asset with the required headers and 404 for the not-yet-built `kernel.wasm`. The full `just build` against the Rust kernel WASM is the real T029 and remains deferred; the `rust-toolchain.toml` in this repo currently targets only `wasm32-unknown-unknown` because `wasm32-wasi` has been removed from the upstream rustup distribution (renamed to `wasm32-wasip1`) and the plan/tasks references need updating in a follow-up.
