...
//! integration test.
//!
//! Trivial toolkit window with shared client-side chrome and body text. The
//! window stays open until its close control is pressed or the server emits
//! `xdg_toplevel.close`.
...
#[cfg(any(target_arch = "wasm32", test))]
const NORMAL_WINDOW_SIZE: (u32, u32) = (300, 160);
    ...
    loop {
        ...
        for event in events {
            match (event.interface, event.opcode) {
                (toolkit::Interface::Pointer, 2) => {
                    let Some((serial, x, y)) = decode_left_pointer_press(&event.payload) else {
                        continue;
                    };
                    let mut frame =
                        sample_window_frame(size, window.is_activated(), window.is_maximized());
                    match frame.pointer_down(x, y) {
        ...
        if let Some(buffers) = pool.as_mut().filter(|buffers| buffers.commit_pending()) {
            if buffers.progress_commit(&mut window)? == toolkit::CommitProgress::Committed {
                log_ready_once(&mut ready_logged);
            }
        }
        ...
        if needs_paint {
            ...
            if focus_changed && window.is_configured() {
                chrome_patch = Some(WindowFramePatch::new(&sample_window_frame(
                    size,
                    was_activated,
                    window.is_maximized(),
                )));
...
#[cfg(any(target_arch = "wasm32", test))]
fn decode_left_pointer_press(payload: &[u8]) -> Option<(u32, i32, i32)> {
    let serial = u32::from_le_bytes(payload.get(0..4)?.try_into().ok()?);
    let x = i32::from_le_bytes(payload.get(8..12)?.try_into().ok()?);
    let y = i32::from_le_bytes(payload.get(12..16)?.try_into().ok()?);
    let button = u32::from_le_bytes(payload.get(16..20)?.try_into().ok()?);
    let state = u32::from_le_bytes(payload.get(20..24)?.try_into().ok()?);
    (button == 1 && state == 1).then_some((serial, x, y))
}

#[cfg(any(target_arch = "wasm32", test))]
fn log_ready_once(ready_logged: &mut bool) {
    if !*ready_logged {
        println!("hello: ready");
        *ready_logged = true;
    }
...
fn paint_window(
    ...
    let background = Color::rgb(0x2a, 0x6c, 0x8a);
    canvas.fill_rect(Rect::new(0, 0, size.0, size.1), background);

    let frame = sample_window_frame(size, focused, maximized);
...
#[cfg(any(target_arch = "wasm32", test))]
fn sample_window_frame(size: (u32, u32), focused: bool, maximized: bool) -> WindowFrame {
    let mut frame = WindowFrame::new(Rect::new(0, 0, size.0, size.1), "Hello, PMos!");
