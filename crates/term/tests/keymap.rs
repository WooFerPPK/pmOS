...
fn unshifted_letter_a_translates_to_lowercase_a() {
    assert_eq!(
        translate_scancode(KEY_A, Modifiers::default()),
        Some(Key::Char('a'))
    );
}
...
fn unshifted_letter_z_translates_to_lowercase_z() {
    assert_eq!(
        translate_scancode(KEY_Z, Modifiers::default()),
        Some(Key::Char('z'))
    );
}
...
fn unshifted_digit_one_translates_to_one() {
    assert_eq!(
        translate_scancode(DIGIT_1, Modifiers::default()),
        Some(Key::Char('1'))
    );
}
...
fn unshifted_digit_zero_translates_to_zero() {
    assert_eq!(
        translate_scancode(DIGIT_0, Modifiers::default()),
        Some(Key::Char('0'))
    );
}
...
fn enter_translates_to_enter_key() {
    assert_eq!(
        translate_scancode(ENTER, Modifiers::default()),
        Some(Key::Enter)
    );
}
...
fn backspace_translates_to_backspace_key() {
    assert_eq!(
        translate_scancode(BACKSPACE, Modifiers::default()),
        Some(Key::Backspace)
    );
}
...
fn space_translates_to_space_char() {
    assert_eq!(
        translate_scancode(SPACE, Modifiers::default()),
        Some(Key::Char(' '))
    );
}
...
fn tab_translates_to_tab_char() {
    assert_eq!(
        translate_scancode(TAB, Modifiers::default()),
        Some(Key::Char('\t'))
    );
}
...
fn shifted_minus_translates_to_underscore() {
    ...
    assert_eq!(
        translate_scancode(MINUS, Modifiers::default()),
        Some(Key::Char('-'))
    );
}
...
fn modifier_keys_translate_to_none() {
    ...
    assert_eq!(
        translate_scancode(CONTROL_RIGHT, Modifiers::default()),
        None
    );
}
...
fn shift_press_then_release_round_trips_state() {
    ...
    assert!(
        was_modifier,
        "shift_left press must be flagged as a modifier"
    );
    ...
    assert!(
        was_modifier,
        "shift_left release must be flagged as a modifier"
    );
