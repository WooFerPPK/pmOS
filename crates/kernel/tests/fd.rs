...
fn alloc_starts_at_zero_and_is_dense() {
    ...
    assert_eq!(t.alloc(FdEntry::new(FdObject::Vnode(1))).unwrap(), 0);
    assert_eq!(t.alloc(FdEntry::new(FdObject::Vnode(2))).unwrap(), 1);
    assert_eq!(t.alloc(FdEntry::new(FdObject::Vnode(3))).unwrap(), 2);
...
fn close_then_alloc_returns_lowest_free_slot() {
    let mut t = FdTable::new();
    let a = t.alloc(FdEntry::new(FdObject::Vnode(1))).unwrap();
    let b = t.alloc(FdEntry::new(FdObject::Vnode(2))).unwrap();
    let c = t.alloc(FdEntry::new(FdObject::Vnode(3))).unwrap();
    assert_eq!((a, b, c), (0, 1, 2));

    // Close fd 1, then alloc: should reuse slot 1, not 3.
    t.close(b).unwrap();
    let d = t.alloc(FdEntry::new(FdObject::Vnode(4))).unwrap();
    assert_eq!(d, 1);
}
...
fn alloc_past_soft_limit_returns_out_of_fds() {
    ...
    t.alloc(FdEntry::new(FdObject::Vnode(1))).unwrap();
    t.alloc(FdEntry::new(FdObject::Vnode(2))).unwrap();
    t.alloc(FdEntry::new(FdObject::Vnode(3))).unwrap();
    let err = t.alloc(FdEntry::new(FdObject::Vnode(4))).unwrap_err();
...
fn close_twice_is_bad_fd() {
    let mut t = FdTable::new();
    let fd = t.alloc(FdEntry::new(FdObject::Vnode(1))).unwrap();
    t.close(fd).unwrap();
...
fn dup_clears_cloexec_on_the_new_entry() {
    let mut t = FdTable::new();
    let fd = t
        .alloc(FdEntry::with_flags(FdObject::Vnode(42), FdFlags::CLOEXEC))
        .unwrap();
    ...
    assert_eq!(t.get(duped).unwrap().object, FdObject::Vnode(42));
}
...
fn install_at_closes_existing_entry_and_returns_it() {
    let mut t = FdTable::new();
    let original = FdEntry::new(FdObject::Vnode(1));
...
fn install_at_to_unopen_slot_grows_table_and_returns_none() {
    let mut t = FdTable::new();
    let closed = t.install_at(3, FdEntry::new(FdObject::Vnode(3))).unwrap();
...
fn install_at_past_soft_limit_is_out_of_fds() {
    ...
    let err = t
        .install_at(5, FdEntry::new(FdObject::Vnode(1)))
        .unwrap_err();
    assert_eq!(err, FdError::OutOfFds);
}
...
fn drop_cloexec_removes_only_cloexec_entries() {
    let mut t = FdTable::new();
    let plain = t.alloc(FdEntry::new(FdObject::Vnode(1))).unwrap();
    let cloexec = t
        .alloc(FdEntry::with_flags(FdObject::Vnode(2), FdFlags::CLOEXEC))
        .unwrap();
    let nonblock = t
        .alloc(FdEntry::with_flags(FdObject::Vnode(3), FdFlags::NONBLOCK))
        .unwrap();

    let dropped = t.drop_cloexec();
    assert_eq!(dropped.len(), 1);
    assert_eq!(dropped[0].0, cloexec);
    assert_eq!(dropped[0].1.object, FdObject::Vnode(2));
...
fn iter_yields_only_open_fds_in_ascending_order() {
    let mut t = FdTable::new();
    t.alloc(FdEntry::new(FdObject::Vnode(10))).unwrap(); // fd 0
    t.alloc(FdEntry::new(FdObject::Vnode(20))).unwrap(); // fd 1
    t.alloc(FdEntry::new(FdObject::Vnode(30))).unwrap(); // fd 2
    t.close(1).unwrap();

    let seen: Vec<(u32, FdObject)> = t.iter().map(|(fd, e)| (fd, e.object)).collect();
    assert_eq!(seen, vec![(0, FdObject::Vnode(10)), (2, FdObject::Vnode(30))]);
}
