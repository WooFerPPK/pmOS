...
// server/client socket pair, drives an IPC_SEND from the client,
// then drives an IPC_RECV from the server (matching the typical
// half-duplex test shape).
//
// Coverage:
//   * happy-path bytes-only round trip (recv_fd_slot = -1).
//   * happy-path bytes + fd round trip with translated FdEntry on
//     the receiver side.
//   * EAGAIN on empty rx_buf + empty rx_fds (v1 doesn't park).
//   * EBADF on non-socket fd (mirrors ipc_send's wrong-fd-kind →
//     EBADF semantic).
//   * EINVAL on flags != 0 (atomic-reject: rx state must not change).
//   * recv_fd_slot = -1 with queued rx_fds leaves them queued for a
//     subsequent recv_fd_slot >= 0 call.

/// Pack `fd: u32` + `len: u32` + `recv_fd_slot: i32` + `flags: u32`
/// into the 16-byte inline-args window the ipc_recv opcode uses.
fn ipc_recv_args(fd: u32, len: u32, recv_fd_slot: i32, flags: u32) -> [u8; 16] {
    let mut args = [0u8; 16];
    args[0..4].copy_from_slice(&fd.to_le_bytes());
    args[4..8].copy_from_slice(&len.to_le_bytes());
    args[8..12].copy_from_slice(&(recv_fd_slot as u32).to_le_bytes());
    args[12..16].copy_from_slice(&flags.to_le_bytes());
    args
}

#[test]
fn ipc_recv_happy_path_drains_bytes_into_heap_out() {
    // Sender writes "hello" via IPC_SEND, receiver drains via
    // IPC_RECV with recv_fd_slot = -1. The response value should
    // equal the payload length, extra_len should equal the payload
    // length (no fd-prefix), and the bytes should land at heap[0..].
    let mut k = make_kernel();
    let server = make_running_proc(&mut k, "srv", 0);
    let client = make_running_proc(&mut k, "cli", 0);
    let (cli_fd, srv_conn) =
        ipc_send_connected_pair(&mut k, client, server, b"/tmp/rcv-happy", 500);
    ...
    let recv_resp = dispatch(
        ...
        &Request {
            ...
            flags: 0,
            request_id: 511,
            args: ipc_recv_args(srv_conn, 64, -1, 0),
            heap_ptr: 0,
            heap_len: 64,
        },
        &mut heap,
    );
    assert_eq!(recv_resp.status, 0);
    assert_eq!(recv_resp.value, payload.len() as i64);
    assert_eq!(recv_resp.extra_len, payload.len() as u32);
    assert_eq!(&heap[..payload.len()], payload);
}

#[test]
fn ipc_recv_with_fd_pass_installs_fd_in_receiver_table() {
    // Client sends "xyz" + fd_to_pass = its /dev/console fd.
    // Server recvs with recv_fd_slot = 0 (any non-negative value
    // means "install in lowest-free slot"). The response should
    // carry the new fd-number at heap[0..4] and the payload at
    // heap[4..7]. The receiver-side fd table should now have a new
    // entry pointing at the same `FdObject::CharDevice(DEV_CONSOLE)`
    // the sender's fd referred to.
    let mut k = make_kernel();
    let server = make_running_proc(&mut k, "srv", 0);
    let client = make_running_proc(&mut k, "cli", 0);
    let (cli_fd, srv_conn) =
    ...
    let recv_resp = dispatch(
        ...
        &Request {
            ...
            flags: 0,
            request_id: 531,
            args: ipc_recv_args(srv_conn, 60, 0, 0),
            heap_ptr: 0,
            heap_len: 64, // 4 (fd prefix) + 60 (payload window)
        },
        &mut heap,
    );
    assert_eq!(recv_resp.status, 0);
    assert_eq!(recv_resp.value, payload.len() as i64);
    assert_eq!(recv_resp.extra_len, 4 + payload.len() as u32);

    let new_fd = u32::from_le_bytes([heap[0], heap[1], heap[2], heap[3]]);
    assert_eq!(&heap[4..4 + payload.len()], payload);

    // The receiver-side fd table now has a new entry at `new_fd`
    // pointing at the same underlying object. Verify by inspecting
    // the receiver's fd table directly.
...
fn ipc_recv_with_flags_one_is_nonblocking_returns_eagain() {
    ...
    let (_cli_fd, srv_conn) =
        ipc_send_connected_pair(&mut k, client, server, b"/tmp/rcv-empty", 540);
...
#[test]
fn ipc_recv_on_non_socket_fd_returns_ebadf() {
    // fd 3 is wired to /dev/console (a CharDevice, not a Socket).
    // ipc_recv must reject with EBADF — distinct from FD_READ on a
    // non-readable fd which uses EINVAL — since IPC opcodes use
    // POSIX `recv(2)`'s "wrong fd kind" → EBADF semantic.
    let mut k = make_kernel();
    let pid = make_running_proc(&mut k, "confused", 0);
    k.install_fd(pid, 3, FdObject::CharDevice(DEV_CONSOLE), FdFlags::EMPTY)
        .unwrap();
...
fn ipc_recv_with_invalid_flag_bit_returns_einval_atomically() {
    ...
    let client = make_running_proc(&mut k, "cli", 0);
    let (cli_fd, srv_conn) =
        ipc_send_connected_pair(&mut k, client, server, b"/tmp/rcv-flags", 570);
    ...
    // Atomic-reject: the byte should still be on the server-side
    // rx_buf (a malformed flag must not silently consume bytes).
    let srv_socket_id = k
        .socket_id_from_fd_public(server, srv_conn)
        .expect("server-side socket id");
    let sock = k
...
#[test]
fn ipc_recv_with_negative_fd_slot_leaves_rx_fds_queued() {
    // Sender pushes payload + ancillary fd. Receiver issues an
    // IPC_RECV with recv_fd_slot = -1 (don't install). Bytes must
    // drain, but rx_fds must stay populated for a subsequent
    // recv_fd_slot >= 0 call.
    let mut k = make_kernel();
    let server = make_running_proc(&mut k, "srv", 0);
    let client = make_running_proc(&mut k, "cli", 0);
    let (cli_fd, srv_conn) =
        ipc_send_connected_pair(&mut k, client, server, b"/tmp/rcv-leaveq", 590);
    ...
    // rx_fds should still hold the queued fd — unconsumed because
    // the caller didn't ask for it. A follow-up IPC_RECV with
    // recv_fd_slot >= 0 should drain it.
    let srv_socket_id = k
        .socket_id_from_fd_public(server, srv_conn)
        .expect("server-side socket id");
    let sock = k
...
#[test]
fn sigint_interrupts_parked_recv_with_eintr() {
    // Ctrl-C semantics: SIGINT (signum 2) interrupts a blocking
    // ipc_recv with -EINTR, mirror of the SIGTERM behaviour. A
    // shell that parks in a syscall while waiting for input must
    // observe ctrl-c without sitting forever — POSIX SIGINT's
    // canonical purpose is interrupting blocking syscalls.
    use abi::cap::{initial, Cap, CapSet};
    let mut k = make_kernel();
    ...
    // Parker slot cleared, A is Ready, block_reason cleared.
    assert!(!k.parked_recvers_contains(a));
    let proc = k.procs.get(a).unwrap();
    assert_eq!(proc.state, ProcState::Ready);
    assert!(proc.block_reason.is_none());
    ...
    // A's signal inbox holds SIGINT — signal delivery runs
    // orthogonally to the park interrupt.
    let drained = k.drain_signals(a).unwrap();
...
#[test]
fn sigint_interrupts_parked_accept_with_eintr() {
    // SIGINT also wakes a parked ipc_accept — same shape as the
    // SIGTERM-on-parked-accept contract. A display server that
    // parks waiting for the next client connect must observe
    // ctrl-c with -EINTR.
    let mut k = make_kernel();
    let ds = k
        .register_process(RegisterArgs {
    ...
    // Listener's parked_acceptor slot cleared via the dispatcher's
    // SIGINT arm calling interrupt_parked_accept.
    ...
    // ds is Ready, block_reason cleared.
    let proc = k.procs.get(ds).unwrap();
    assert_eq!(proc.state, ProcState::Ready);
    assert!(proc.block_reason.is_none());

    // Two wakes can land here: the EINTR for the parked accept
    // (queued by interrupt_parked_accept) AND the proc_kill
    // success Response (the dispatcher's Done outcome). The
    // EINTR is what the test cares about.
    let wakes = k.pending_wakes_snapshot();
    let eintr_wake = wakes
        .iter()
        .find(|(pid, resp, _)| *pid == ds && resp.request_id == req_id)
        .expect("EINTR wake on the parked-accept req_id");
    assert_eq!(eintr_wake.1.status, -errno::EINTR);

    // SIGINT queued on ds's signal inbox.
    let drained = k.drain_signals(ds).unwrap();
    assert_eq!(drained, alloc::vec![Signal::Interrupt]);
}

#[test]
fn sigint_interrupts_parked_wait_with_eintr() {
    // SIGINT also interrupts a blocking proc_wait — a parent
    // parked waiting on a child must observe ctrl-c with -EINTR
    // rather than sitting forever.
    let mut k = make_kernel();

    let init = make_running_proc(&mut k, "init", 0);

    // Parent has init as ppid so the cap check in proc_kill
    // passes via the parent-relationship arm.
    let parent = register_child(&mut k, init, "parent");
    k.procs.transition(parent, ProcState::Running).unwrap();
    // Parent has its own live child to wait on.
    ...
    let mut kill_args = [0u8; 16];
    kill_args[0..4].copy_from_slice(&(parent as i32).to_le_bytes());
    kill_args[4..6].copy_from_slice(&2u16.to_le_bytes());
    let mut heap = vec![0u8; 16];
    let kill_resp = dispatch(
        &mut k,
    ...
    // Parent is Ready, block_reason cleared, parker slot drained.
    let proc = k.procs.get(parent).unwrap();
    assert_eq!(proc.state, ProcState::Ready);
    assert!(proc.block_reason.is_none());
    assert!(k.parked_waiters_get_public(parent).is_none());

    // Exactly one EINTR wake on parent's request_id.
    let wakes = k.pending_wakes_snapshot();
    let eintr_wake = wakes
        .iter()
        .find(|(pid, resp, _)| *pid == parent && resp.request_id == req_id)
        .expect("EINTR wake on the parked-wait req_id");
    assert_eq!(eintr_wake.1.status, -errno::EINTR);
    assert_eq!(eintr_wake.1.extra_len, 0);

    // SIGINT queued.
    let drained = k.drain_signals(parent).unwrap();
    assert_eq!(drained, alloc::vec![Signal::Interrupt]);
}

#[test]
fn sigpipe_does_not_interrupt_parked_recv() {
    // SIGPIPE (13) is a write-side notification — it has no POSIX
    // analogue for interrupting blocking read syscalls. The
    // dispatcher's catchable-signal arm restricts park-interrupt
    // to {Term, Interrupt}; Pipe is queued on the inbox but the
    // parker stays asleep.
    use abi::cap::{initial, Cap, CapSet};
    let mut k = make_kernel();
    ...
    // SIGPIPE queued on A's inbox.
    let drained = k.drain_signals(a).unwrap();
    assert_eq!(drained, alloc::vec![Signal::Pipe]);
}

#[test]
fn sigchld_does_not_interrupt_parked_recv() {
    // SIGCHLD (17) is a child-exit notification delivered TO a
    // parent — interrupting an unrelated park on Child arrival
    // would be surprising (POSIX gates SIGCHLD's blocking-
    // syscall interruption behind SA_RESTART, which v1 doesn't
    // model). Dispatcher's catchable-signal arm leaves Child out
    // of the {Term, Interrupt} interrupt set.
    use abi::cap::{initial, Cap, CapSet};
    let mut k = make_kernel();
